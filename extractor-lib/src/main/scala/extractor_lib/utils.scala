...
object Utils {
  ...
  def readVirtualFile(fileName: String): VirtualFile = {
    val path = java.nio.file.Paths.get(".", fileName)
    val bytes = java.nio.file.Files.readAllBytes(path)
    val text = new String(bytes, "UTF-8")
    val input: VirtualFile = VirtualFile(path.toString, text)
    input
  }

  /** https://gist.github.com/carymrobbins/7b8ed52cd6ea186dbdf8
    *
    * Pretty prints a Scala value similar to its source representation. Particularly useful for case classes.
    * @param a
